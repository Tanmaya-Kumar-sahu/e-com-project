...
const Cart = () => {
    ...
    const{products,currency,cartItem,upadateQuantity,navigate}=useContext(ShopContext)
          ...
          cartData.map((item,index)=>{
            ...
            return(
              <div key={index} className='py-4 border-t border-b text-gray-700 grid grid-cols-[4fr_0.5fr_0.5fr] sm:grid-cols-[4fr-2fr-0.5fr] items-center gap-4'>
                  ...
                  <img onClick={()=>upadateQuantity(item._id,item.size,0)} src={assets.bin_icon} className='w-4 mr-4 sm:w-5 cursor-pointer' alt="" />
              </div>
            )
          })
        }
